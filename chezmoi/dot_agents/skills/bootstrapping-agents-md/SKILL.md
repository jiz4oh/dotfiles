...
- 使用标题、列表和简短代码块
- 只保留高信号的仓库事实和工作策略
- 任何准备写入 `AGENTS.md` 的内容都必须先归类为 `Confirmed Policies`、`Inferred Repo Facts` 或“暂不写入”
- 未确认的用户偏好不得伪装成仓库事实
- 默认选择最小可用解；可选章节只有在确实能帮助未来 agent 决策时才加入
...
每个关卡都要先产出中间结果，再进入下一步，避免一口气生成整份文件。
...
- 运行：
  ...
  ```
- 先把扫描结果拆成三类：`Confirmed Policies`、`Inferred Repo Facts`、`待确认`
- 扫描脚本只提供候选证据，不直接决定最终写入内容
...
- 已有 `AGENTS.md` 时默认做手术式 patch
- 每一处变更都要能追溯到“事实漂移”或“用户新确认策略”
...
- 是否存在无证据断言
- 是否和 `README`、CI 或现有配置冲突
- 是否引入了无关风格化改写

## 输出格式要求

- 生成前必须先列 `已确认`、`推断`、`待确认` 三类信息
- 生成时必须说明本次省略了哪些可选章节，以及为什么省略
- refine 或 pattern patch 时必须显式标注这是“事实漂移”还是“策略漂移”
- 无法解释“为什么要改这行”时，就不要把那一行写进 patch
