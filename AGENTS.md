...
- 外部依赖仓库由 `chezmoi/.chezmoiexternal.toml` 管理。
...
- 改 `chezmoi/Brewfile`、`chezmoi/.chezmoiexternal.toml`、`chezmoi/run_*`、`chezmoi/dot_*` 后，至少运行 `chezmoi --source "$PWD" managed` 和 `chezmoi --source "$PWD" apply --dry-run --force --no-tty`。
...
- 外部仓库优先通过 `.chezmoiexternal.toml` 管理，避免直接把第三方源码散落进受管目录。
